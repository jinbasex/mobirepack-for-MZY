...
def get_ordered_images_from_extracted_mobi(extract_dir):
    ...
    print(f"成功破译顺序！共锁定 {len(ordered_images)} 张高清原图。")
...
def process_single_book(input_path, kindlegen_path, base_dir):
    ...
    try:
        ...
        try:
            ...
            # 4. 生成 HTML/OPF
            print(f"有效页数: {len(valid_images)}。正在生成元数据...")
            html_files = []
            ...
            opf_content = ['<?xml version="1.0" encoding="utf-8"?>',
                           ...
                           f'<metadata xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:title>{book_title}</dc:title><dc:language>zh</dc:language><meta name="fixed-layout" content="true"/><meta name="original-resolution" content="{KINDLE_WIDTH}x{KINDLE_HEIGHT}"/><meta name="book-type" content="comic"/><meta name="zero-margin" content="true"/></metadata><manifest>',
            ...
            output_target = os.path.join(target_dir, output_name)
